            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        ...
                        <!-- form start -->
                        <form method="POST" id="editCompanyForm" action="{{ route('employee.update', $employee) }}"
                              enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" id="companyId" name="id" value="{{$employee->id}}">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="phone" class="form-control" id="phone"
                                           placeholder="Enter employee phone number" value="{{$employee->phone}}">
                                    @error('phone')
                                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="company_id" class="form-control" id="companyId">
                                        @foreach($companies as $c)
                                            <option
                                                value="{{$c->id}}" {{($c->id===$employee->company_id) ? 'selected':''}}>{{$c->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('company_id')
                                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
